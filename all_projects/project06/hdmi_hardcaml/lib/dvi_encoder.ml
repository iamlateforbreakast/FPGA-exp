...
let create (scope : Scope.t) (input : _ EncodeInput.t) =
  ...
  let count_ones (d : Signal.t) = 
    ...
    |> List.fold_left (fun acc bit -> acc +: (zero_extend bit (width acc))) (of_int ~width:4 0)
  in
  ...
  compile [
    if_ ~: (input.de) [
      ...
      encoded <-- switch input.control [
        of_int ~width:2 0, of_int ~width:10 0b1101010100;
        of_int ~width:2 1, of_int ~width:10 0b0010101011;
        of_int ~width:2 2, of_int ~width:10 0b0101010100;
        of_int ~width:2 3, of_int ~width:10 0b1010101011;
      ];
    ] [
